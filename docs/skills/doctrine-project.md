...
Every Done means item, and every end-state item, passes one admission test before it is put to you: "Can [requirement] be objectively measured/verified?" If not, it becomes an owner-judgement item that names you as the judge. An item that joins two requirements is split into two. Each item carries seven fields: an ID, the outcome it names, its type, the check a non-builder runs, a control showing that check can fail, the evidence it requires, and which epic or phase covers it. The four types are T1 behaviour, T2 artifact property, T3 threshold with a pass line, and T4 owner judgement.
...
**States.** An epic is Proposed, Not started, Open, Done, Dropped or Superseded. Your ruling on its Done means moves it from Proposed to Not started, and its first member phase opens it. It reaches Done only on an exit return from a context that did not build the work, with a PASS and evidence for every Done means item, against the epic's current baseline, on the recorded certification target. A PASS with no evidence counts as unverified. A later regression against a Done means item reopens the epic. The project is Proposed until you rule its end state, then Ruled, and Done only when every epic on the roster is Done, Dropped or Superseded and a project exit return passes every end-state item.
