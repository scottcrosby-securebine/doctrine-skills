...
**Overflow and exits.** Six is the whole column. Past six, extras open as tabs in herdr's sidebar. When an ordinary agent finishes its pane closes itself, unless you are reading it at that moment, in which case it stays with a `· done` suffix. A Codex pane is the exception: it relabels with the job's final status and stays, so you can read the result, and a finished unfocused one closes when the next Codex seat is placed or at session cleanup. Nothing ever steals your cursor: panes and tabs open unfocused.
