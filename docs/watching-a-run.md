...
The separator is U+00B7, the character the token carries, and `{ contains = "·v0" }` with no style is a deliberate stop: it keeps the default look for a run that has found nothing yet. On 0.8.2 drop `rules` and keep the rest; the value shows in one colour. Run `herdr config check`, then `herdr server reload-config`. The row appears the next time a round is recorded and expires an hour after the last one, so an empty row after a finished run is not a config fault.
