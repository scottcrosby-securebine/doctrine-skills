...
**The counter token.** Each gate round the orchestrator publishes one token, `$doctrine`, whose value is `r<round>·e<exit>·v<valve>`: the round number, `e1` once a clean pass is on record for the current revision and `e0` otherwise, and the count of rounds that found a blocking finding. It renders only if your herdr `config.toml` names it in an Agent row. On herdr 0.9.0 or later, rules on that row colour it: green when a clean pass stands, red the moment a round has found a blocker and no clean pass has come since, and unstyled before any round has found one. First matching rule wins, so the order below is the logic.
...
The separator is U+00B7, the character the token carries, and `{ contains = "·v0" }` with no style is a deliberate stop: it keeps the default look for a run that has found nothing yet. On 0.8.2 write `"$doctrine"` in place of the table; the value shows in one colour. Run `herdr config check`, then `herdr server reload-config`, and the row appears the next time a round is recorded. Nothing in the run reads the token back, so a config mistake costs the colour and nothing else.
