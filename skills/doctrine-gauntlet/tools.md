...
```bash
herdr pane split --current --direction right --ratio 0.4 --no-focus --cwd "$PWD" --env HISTFILE=~/.dctr_history   # note .result.pane.pane_id; the env keeps the seat's shell history out of the user's
