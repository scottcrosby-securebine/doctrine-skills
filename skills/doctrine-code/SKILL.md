...
4. Phase exit: native checks → `matts-code-review` → red team → simplification review once per phase (doctrine step 6: before the certifying passes) → loop to two consecutive clean passes.
...
Same flow, plus the following — **and put it in each wave agent's prompt, not only here. The agent that styles the component is a wave agent; a rule sitting where only the dispatcher reads it reaches nobody who writes CSS.**
