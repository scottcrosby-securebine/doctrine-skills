...
- **Where the record lives.** The run record for a phase goes under the path on the project file's `Records:` line, excluded from version control, per doctrine step 1. Before a project file exists, it goes under `.doctrine/records/` in the repo root, excluded through `.git/info/exclude` so no tracked file changes, and the project file that start or adopt writes carries `Records: .doctrine/records` unless the repo's own conventions name another path. `docs/PROJECT.md` and `docs/epics/<ID>.md` are tracked contracts, not the record. Never gitignore them.
...
4. Give every open issue, and every open item read from a markdown tracker, a destination: a member of an epic, out of scope, or the post-done backlog. Each destination is a `destination` ruling and a row in `## Open issues`.
...
Every item has the seven fields `formats.md` defines. Before any item goes to the owner as part of a baseline, apply the admission test to it: "Can [requirement] be objectively measured/verified?" If not, it is type T4, owner judgement: its check verifies that the owner's ruling on a named revision exists, never the taste. Then ask, at the revision the item is admitted at: does the item's Check already pass there? If it does, the item is not live Done means: it is history, or the owner rules it `preserve` for the phase that must keep it passing. Split a conjunction into separate items. An item missing any field is not put to the owner.
