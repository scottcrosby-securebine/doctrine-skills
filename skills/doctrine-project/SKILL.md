...
- **W3.** The `impact` ruling "A Done means change" above requires is the owner's, because the owner is the one who accepts that the evidence behind the changed items is now stale.
...
4. **Record.** You write the return entry as the seat gave it, with the Baseline it was briefed on, even when a `done-means-change` ruling landed while it worked. A return missing any item's result is a failed pass, and so is an epic return missing the `Combined check result:` line step 3 requires. Read what counts from `formats.md`.
