...
Every item has the seven fields `formats.md` defines. Before any item goes to the owner as part of a baseline, apply the admission test to it: "Can [requirement] be objectively measured/verified?" If not, it is type T4, owner judgement: its check verifies that the owner's ruling on a named revision exists, never the taste. Then, at the revision the item is admitted at, run its Control and its Check. A Control that does not fail there means the Check cannot fail: repair the Check and its Control before the item goes anywhere. Only once the Control fails does a Check that already passes mean the item is not live Done means: it is then history, or the owner rules it `preserve` for the phase that must keep it passing. Split a conjunction into separate items. An item missing any field is not put to the owner.
