...
4. Edit: fix Accuracy findings, write Coverage gaps. Match each doc's existing voice and structure; use writing-clearly-and-concisely if installed. Simplicity applies to prose too: delete redundant docs rather than maintaining duplicates. Treat agent-instruction files (CLAUDE.md, AGENTS.md) with extra care: edits there change agent behavior, so flag them separately in the PR.
