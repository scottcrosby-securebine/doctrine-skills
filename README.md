...
5. **Loop until confident**: a phase exits on two consecutive clean passes of the full gate. Any diff change resets the counter to zero, simplification included. A separate valve counts unresolved rounds and never resets, escalating at every multiple of four and re-arming rather than being spent; both counters go to disk, because a compaction zeroes an in-context counter and tells nobody. A phase that ends any other way — stopped by the user, shipped at an escalation, ended with a direction question unanswered — is recorded as that state, never as clean, and a phase still running is recorded as open, so that a record with no state means the write was lost. A finding caused by the phase's own earlier change is fallout: it blocks like any other, but it is fixed as a class. And where **any** class of finding can be caught mechanically, the check is built before another review round runs. The valve's escalation reports where the findings landed and why, not a bare count.
6. **Simplify early and often**: a YAGNI/dead-code pass once at each phase exit — and not again on the re-entry passes its own diff caused, or the phase can never close; reuse before reinventing.
7. **Deliver**: work ends with the repo's delivery norm, never with "the loop is clean." Commit — then push or open a PR **only where that norm says to, or where the user has said to**; where nothing documents a norm there is nothing to defer to, so commit locally and ask. Close by walking the original request item by item and naming where each one landed.
