...
5. **Loop until confident**: a phase exits on two consecutive clean passes of the full gate. Any diff change resets the counter to zero, simplification included. A separate valve counts unresolved rounds and never resets, escalating at every multiple of four and re-arming rather than being spent; both counters go to disk, because a compaction zeroes an in-context counter and tells nobody. A phase that ends any other way — stopped by the user, shipped at an escalation, ended with a direction question unanswered — is recorded as that state, never as clean. A finding caused by the phase's own earlier change is fallout: it blocks like any other, but it is fixed as a class, and where the class can be caught mechanically the check is built before another review round runs. The valve's escalation reports where the findings landed and why, not a bare count.
