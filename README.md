...
**Tidy exits.** When an agent finishes, its pane or tab closes itself. If you happen to be reading it at that moment it stays, renamed with a `· done` suffix, so nothing disappears while you are looking at it. One seat is different: a codex rescue agent returns in about a minute while the job it started runs on, so its pane follows that job's log instead and stays on screen, renamed with the job's final status, until the session ends or you close it. A retained side pane holds one of the six side slots; an overflow tab does not. And nothing ever steals your cursor: panes and tabs open unfocused.
