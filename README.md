...
- **How these are tested.** The skills are markdown, so there is no suite to run. Two things stand in for one. Fresh-context agents are run through each wrapper against realistic scenarios and every ambiguity they hit is patched — that is validate-then-repair, not TDD, and it is periodic rather than per-commit, so edits can be ahead of the last full run. And [`fixtures/`](fixtures/) is a standing regression suite for `doctrine-gauntlet`: three generality cases, each proving something the other two cannot, run against any change to the gauntlet or its harness.
