...
console.log('the overflow ruling, driven through open rather than asserted around it')
{
  ...
  check('and the split carries the launcher cwd', herdrSaw(new RegExp(`^pane split .* --cwd ${process.cwd().replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}( |$)`, 'm')), fs.readFileSync(calls, 'utf8').trim())
  ...
  check('and the tab create carries the launcher cwd', herdrSaw(new RegExp(`^tab create .* --cwd ${process.cwd().replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}( |$)`, 'm')), fs.readFileSync(calls, 'utf8').trim())
