...
const FORMS = [
  ...
  [form('auto-cycle:\\s+on\\s+cap\\s+(\\d+)\\s+tier\\s+(\\S+)\\s*'), (m) => ({ kind: 'auto-cycle', sub: 'on', cap: num(m[1]), tier: m[2] })],
  [form('auto-cycle:\\s+off\\s*'), () => ({ kind: 'auto-cycle', sub: 'off' })],
  ...
  [form('auto-cycle:\\s+ready\\s*'), () => ({ kind: 'auto-cycle', sub: 'ready' })],
  [form('auto-cycle paused:\\s+(\\S.*)'), (m) => ({ kind: 'auto-cycle', sub: 'paused', reason: m[1].trim() })],
]

/** `wrapper: <name>`, key in any case, as a list item or bare, optionally bold. The value is the first
 *  whitespace-delimited token after the key with backticks, `**`, a `doctrine:` prefix and a trailing period
