...
const MUTATIONS = [
  ...
  { name: "a finished gate looks for its marker only at the original path, so a moved one is never dropped (B4)", file: "dctr-gate.mjs",
    clause: "clause 9 —",
    from: "[marker, moved].find((f) => fs.existsSync(f))",
    to: "[marker].find((f) => fs.existsSync(f))" },
  { name: "a finished gate reads the moved path before the original (B4)", file: "dctr-gate.mjs",
    clause: "clause 9d",
    from: "[marker, moved].find((f) => fs.existsSync(f))",
    to: "[moved, marker].find((f) => fs.existsSync(f))" },
