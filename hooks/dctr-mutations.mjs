...
const SUITES = ['dctr-record.selftest.mjs', 'dctr-project.history.selftest.mjs', 'dctr-project.selftest.mjs', 'dctr-restore.selftest.mjs', 'dctr-bridge.selftest.mjs', 'dctr-seat.selftest.mjs', 'dctr-gauge.selftest.mjs', 'dctr-typer.selftest.mjs', 'dctr-cycle.selftest.mjs', 'dctr-gate.selftest.mjs', 'dctr-pane.selftest.mjs', 'dctr-seat.teardown.selftest.mjs']
...
const MUTATIONS = [
  ...
  { name: "any session's warned line counts as this session's (E8-D7)", file: "dctr-cycle.mjs",
    ...
    from: "e.sub === 'warned' && e.session === sessionId).at(-1)",
    to: "e.sub === 'warned').at(-1)" },
  ...
  { name: "the pane message is printed at every event (E8-R23)", file: "dctr-state.mjs",
    ...
    to: "return true" },
  { name: "step 3 counts a resolved pause after the warned line (E8-R25)", file: "dctr-cycle.mjs",
    clause: "clause 2c4",
    from: "unresolvedPausesAfter(record.entries, mine.line).length > 0",
    to: "record.entries.some((e) => e.sub === 'paused' && e.line > mine.line)" },
  { name: "the token reads on while a pause stands behind the ready line (RB2-3)", file: "dctr-cycle.mjs",
    clause: "clause 2c8",
    from: "publishToken(paneId, standing.length ? pausedToken(standing.at(-1).reason)",
    to: "publishToken(paneId, false ? pausedToken(standing.at(-1).reason)" },
  ...
  { name: "the dedup holds on any latest paused line, answered or not (E8-R25)", file: "dctr-lib.mjs",
    clause: "clause 2c5",
    from: "  return latest?.sub === 'paused' && standingPauses(entries, startLine).includes(latest)",
    to: "  return latest?.sub === 'paused'" },
  ...
  { name: "a paused line is written over one already standing (E8-D18)", file: "dctr-state.mjs",
    clause: "clause 1k",
    from: "  if (pauseStands(parseRecord(fs.readFileSync(recordPath, 'utf8')).entries, sessionStartLine(recordPath))) return { written: false }\n",
    to: "" },
  ...
  { name: "the Stop hook reads the token's pauses without the session start (RB3-2)", file: "dctr-cycle.mjs",
    clause: "clause 2c10",
    from: "  const standing = standingPauses(entries, sessionStartLine(recordPath))\n",
    to: "  const standing = standingPauses(entries)\n" },
