...
const MUTATIONS = [
  ...
  { name: "the typer resumes before herdr reports the new session (E8-D15)", file: "dctr-lib.mjs",
    clause: "clause 1g",
    from: "  if (o.pane.session === o.restore.session) return { act: 'resume'",
    to: "  if (true) return { act: 'resume'" },
  ...
  { name: "the pane message leaves out what to do (E8-R23)", file: "dctr-lib.mjs",
    clause: "clause 1d",
    from: "export const pauseMessage = (reason) => `doctrine auto-cycle paused: ${reason}. ${pauseAction(reason)}`",
    to: "export const pauseMessage = (reason) => `doctrine auto-cycle paused: ${reason}`" },
  ...
  { name: "the Stop facts are not persisted for idle_prompt (S4)", file: "dctr-cycle.mjs",
    ...
    to: "    ({" },
  { name: "any session's warned line counts as this session's (E8-D7)", file: "dctr-cycle.mjs",
    clause: "another session's warned line only",
    from: "e.kind === 'auto-cycle' && e.sub === 'warned' && e.session === sessionId)",
    to: "e.kind === 'auto-cycle' && e.sub === 'warned')" },
  ...
  { name: "the standing pauses ignore the recorded session start (RB3-2)", file: "dctr-lib.mjs",
    clause: "clause 1q2",
    from: "  const anchor = Math.max(marked, startLine)\n",
    to: "  const anchor = marked\n" },
  ...
  { name: "two lines are the same pause only with the same text (E8-R27)", file: "dctr-lib.mjs",
    ...
    to: "a === b" },
  { name: "the skills' question pause asks only for an answer in the pane (S4-1)", file: "dctr-lib.mjs",
    clause: "clause 2j",
    from: "(SKILL_QUESTION.test(reason) ? 'answer in the pane, then /clear and type resume' : 'answer in the pane')",
    to: "'answer in the pane'" },
  { name: "the toast reads the standing pauses without the session start (F1)", file: "dctr-state.mjs",
    clause: "clause 1s",
    from: "const fresh = standingPauses(entries, start).filter",
    to: "const fresh = standingPauses(entries).filter" },
  { name: "the pane message reads the standing pauses without the session start (F1)", file: "dctr-state.mjs",
    clause: "clause 1s2",
    from: "  const standing = standingPauses(parseRecord(fs.readFileSync(recordPath, 'utf8')).entries, sessionStartLine(recordPath))\n",
    to: "  const standing = standingPauses(parseRecord(fs.readFileSync(recordPath, 'utf8')).entries)\n" },
]
