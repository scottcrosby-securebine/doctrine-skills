...
const MUTATIONS = [
  ...
  { name: "the dedup holds on any paused line of the same pause, answered or not (E8-R25)", file: "dctr-lib.mjs",
    clause: "clause 2c5",
    from: "reason) => standingPauses(entries, startLine).some((p) => samePause(p.reason, reason))",
    to: "reason) => (entries || []).some((p) => p.sub === 'paused' && samePause(p.reason, reason))" },
  ...
  { name: "the dedup refuses any pause while the latest auto-cycle line is a standing one (RB4-2)", file: "dctr-lib.mjs",
    clause: "clause 2c12",
    from: "reason) => standingPauses(entries, startLine).some((p) => samePause(p.reason, reason))",
    to: "reason) => { const l = latestAutoCycle(entries); return l?.sub === 'paused' && standingPauses(entries, startLine).includes(l) }" },
