...
const MUTATIONS = [
  ...
  { name: "the dedup holds on any paused line of the same pause, answered or not (E8-R25)", file: "dctr-lib.mjs",
    clause: "clause 2c5",
    from: "  const standing = standingPauses(entries, startLine)\n  return pauseCode(reason) === 'R8'",
    to: "  const standing = (entries || []).filter((p) => p.sub === 'paused')\n  return pauseCode(reason) === 'R8'" },
  ...
  { name: "a paused line is written over one already standing (E8-D18)", file: "dctr-state.mjs",
    clause: "clause 1k",
    from: "  if (pauseStands(parseRecord(fs.readFileSync(recordPath, 'utf8')).entries, sessionStartLine(recordPath), reason)) return { written: false }\n",
    to: "" },
  { name: "the recorded session start is never read (RB3-2)", file: "dctr-state.mjs",
    clause: "clause 2c10",
    from: "return Number.isInteger(n) ? n : 0 } catch { return 0 }",
    to: "return 0 } catch { return 0 }" },
