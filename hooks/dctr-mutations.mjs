...
const MUTATIONS = [
  ...
  { name: "the ready line counts anywhere in the message, not only as its last line", file: "dctr-lib.mjs",
    clause: "clause 1h2",
    from: "unwrapLine(String(message ?? '').trimEnd().split('\\n').at(-1)) === READY_LINE",
    to: "String(message ?? '').includes(READY_LINE)" },
  { name: "an empty background_tasks or session_crons object counts as live work", file: "dctr-lib.mjs",
    clause: "clause 1h2",
  ...
  { name: "a reply with no Claude session reads as a changed session, R16 (ST2)", file: "dctr-lib.mjs",
    clause: "clause 1e3",
    from: "  if (paneSession(o.pane) === null) return pause('R17', R17_WHY.lookup)",
    to: "  if (!o.pane || o.pane.error) return pause('R17', R17_WHY.lookup)" },
  ...
  { name: "the typer never pauses for a missing restore file (R14)", file: "dctr-lib.mjs",
    clause: "clause 1f",
    from: "    if (o.waited < t.restore) return { act: 'wait', reason: 'waiting for the restore file' }\n",
    to: "    return { act: 'wait', reason: 'waiting for the restore file' }\n" },
  ...
  { name: "a claim counts as held whatever its pid (S3)", file: "dctr-state.mjs",
    ...
    to: "  return true\n" },
  { name: "the claim lookup never counts a held claim, so idle_prompt pauses while the typer holds one for this pane (F9)", file: "dctr-state.mjs",
    clause: "clause 2o",
  ...
  { name: "the token is republished when unchanged (LB2)", file: "dctr-state.mjs",
    ...
    to: "" },
  { name: "the pane message shares the toast's marker, which the toast takes first, so no paused line's pane message is ever printed (SP1)", file: "dctr-state.mjs",
    clause: "clause 2c7",
  ...
  { name: "the auto-cycle hook calls herdr in a contained session, on Stop, Notification and StopFailure (E8-D26)", file: "dctr-cycle.mjs",
    clause: "clause 2e",
    from: "  const paneId = contained ? null : process.env.HERDR_PANE_ID || null",
    to: "  const paneId = process.env.HERDR_PANE_ID || null" },
  { name: "the auto-cycle hook reads only the session's repo, so it neither finds the stop file in the record's repo nor hashes that repo's tree (Q1)", file: "dctr-cycle.mjs",
    clause: "clause 2c",
  ...
  { name: "the launch prints no pane message (E8-R23)", file: "dctr-cycle.mjs",
    ...
    to: "" },
  { name: "a Notification or a StopFailure acts while the stop file stands (B6)", file: "dctr-cycle.mjs",
    clause: "clause 2n2",
    from: "  } else if (active) {",
    to: "  } else if (true) {" },
  { name: "the hooks.json description says the entries stand down unless auto-cycle is active, the stop file included (RB3)", file: "hooks.json",
    clause: "clause 2r2",
    from: "each stands down, as does a subagent's event, unless that record is Open or Blocked and its last auto-cycle on/off line is on.",
    to: "each stands down unless auto-cycle is active, meaning no .doctrine/auto-cycle.stop exists." },
  { name: "clause 2r2's guard loses its PAUSES-derived half, so a D2 reason written into the description passes (RB12-1)", file: "dctr-cycle.selftest.mjs",
    clause: "clause 2r2",
    from: "const FORBIDDEN = [...D2_REASONS, ...D2_FIXED,",
    to: "const FORBIDDEN = [" },
  { name: "the hooks.json description restates the Stop's pause reasons (DR11)", file: "hooks.json",
    clause: "clause 2r2",
    from: "UserPromptSubmit only records where the session's turn ended.",
    to: "UserPromptSubmit only records where the session's turn ended. A ready Stop pauses when the handoff is not written or the cycle cap is reached." },
  { name: "the typer leaves an earlier restore file in place before /clear", file: "dctr-typer.mjs",
    ...
    to: "" },
  // Moved with E8-D28: the cycle line is no longer written at the send, so this reverts LN12 by writing it before it.
  { name: "the cycle line records an intended /clear, not a sent one (LN12)", file: "dctr-typer.mjs",
    clause: "clause 2q",
    from: "      fs.rmSync(restoreFile(a.pane), { force: true })\n      send('/clear')\n",
    to: "      fs.rmSync(restoreFile(a.pane), { force: true })\n      appendRecordLine(a.record, `- auto-cycle: cycle ${a.n} tree ${a.hash}`)\n      send('/clear')\n" },
  ...
  { name: "an unreadable old transcript reads as no new entries (RB2)", file: "dctr-typer.mjs",
    ...
    to: "could not be read (${e.code || e.message})`); return { entries: [], partial: false } }" },
  ...
  { name: "the typer never sees a paused line written since its claim", file: "dctr-typer.mjs",
    clause: "clause 2r",
    from: "e.sub === 'paused' && e.line > a.keyLine)",
  ...
  { name: "a pause is judged against every line of the record, so an answer written before it resolves it (E8-R25)", file: "dctr-lib.mjs",
    clause: "clause 1p",
    from: "const after = (entries || []).filter((e) => e.line > paused.line)",
    to: "const after = (entries || []).filter(() => true)" },
  { name: "an alarm pause resolves only while no later alarm is unruled, so a later alarm makes an answered one stand again (RB2-1)", file: "dctr-lib.mjs",
    clause: "clause 1p2",
    from: "  if (PAUSES.R3.match.test(reason)) return after.some((e) => e.kind === 'ruling')\n",
    to: "  if (PAUSES.R3.match.test(reason)) return after.some((e) => e.kind === 'ruling') && !(entries || []).some((x) => x.kind === 'alarm' && !(entries || []).some((r) => r.kind === 'ruling' && r.line > x.line))\n" },
  { name: "the typer's claim baseline is a fresh read of the record, so a pause written before it starts is missed (RB8-1)", file: "dctr-typer.mjs",
    clause: "clause 2r2",
    from: "  writeMarker(claimFile(a.session, a.keyLine), { pid: process.pid, pane: a.pane })\n",
    to: "  writeMarker(claimFile(a.session, a.keyLine), { pid: process.pid, pane: a.pane })\n  { const t = fs.readFileSync(a.record, 'utf8'); a.keyLine = t.split('\\n').length - (t.endsWith('\\n') ? 1 : 0) }\n" },
  { name: "the hook reads stdin outside its guarded path, so an unreadable stdin exits 1 with a stack (RB8-2)", file: "dctr-cycle.mjs",
    clause: "clause 2t",
    from: "if (isMain) runHook(() => fs.readFileSync(0, 'utf8'))",
    to: "if (isMain) runHook(fs.readFileSync(0, 'utf8'))" },
  ...
  { name: "the token never reads paused: it reads on while any pause stands (RB2-3)", file: "dctr-lib.mjs",
    clause: "clause 2c8",
  ...
  { name: "a paused line is written over one already standing (E8-D18)", file: "dctr-state.mjs",
    clause: "clause 1k",
    from: "  if (pauseStands(parseRecord(fs.readFileSync(recordPath, 'utf8')).entries, sessionStartLine(recordPath), reason, sessionId ? turnEndLine(sessionId, recordPath) : 0)) return { written: false }\n",
    to: "" },
  ...
  { name: "the typer runs though its claim is already taken (E8-D15)", file: "dctr-typer.mjs",
    clause: "clause 2b",
  ...
  { name: "a stop file found by the typer aborts without R1 (RN3-3)", file: "dctr-lib.mjs",
    ...
    to: "  if (o.stopRepo) return { act: 'abort', reason: 'a stop file exists' }\n" },
  ...
  { name: "an R7, R8 or R9 from before the session's latest turn end still counts in the dedup (E8-R29, RB5-2)", file: "dctr-lib.mjs",
    clause: "clause 2c14",
    from: ".filter((p) => !(EVENT_PAUSES.includes(pauseCode(p.reason)) && p.line <= turnEnd))",
    to: "" },
  { name: "every pause, not only R7, R8 and R9, is freed by a turn end (E8-R29)", file: "dctr-lib.mjs",
    clause: "clause 1v",
    from: "const EVENT_PAUSES = ['R7', 'R8', 'R9']",
    to: "const EVENT_PAUSES = Object.keys(PAUSES)" },
  { name: "the hook's pauses are deduped without the session's latest turn end (E8-R29)", file: "dctr-cycle.mjs",
    clause: "clause 2c14",
    from: "appendPaused(recordPath, reason, sessionId)",
    to: "appendPaused(recordPath, reason)" },
  { name: "a Stop records no turn end (E8-R29)", file: "dctr-cycle.mjs",
    clause: "clause 2c14",
    from: "  if (event !== 'Notification') writeTurnEnd(sessionId, recordPath)\n",
    to: "  if (event !== 'Notification' && event !== 'Stop') writeTurnEnd(sessionId, recordPath)\n" },
  { name: "a StopFailure records no turn end (E8-R31, DP6-B1)", file: "dctr-cycle.mjs",
    clause: "clause 2c15",
    from: "  if (event !== 'Notification') writeTurnEnd(sessionId, recordPath)\n",
    to: "  if (event !== 'Notification' && event !== 'StopFailure') writeTurnEnd(sessionId, recordPath)\n" },
  { name: "a submitted prompt records no turn end (E8-R31, DP6-B1)", file: "dctr-cycle.mjs",
    clause: "clause 2c15",
    from: "  if (event !== 'Notification') writeTurnEnd(sessionId, recordPath)\n",
    to: "  if (event !== 'Notification' && event !== 'UserPromptSubmit') writeTurnEnd(sessionId, recordPath)\n" },
  { name: "a StopFailure records its turn end after its own pause too, freeing its own R9 at once (RB7-3)", file: "dctr-cycle.mjs",
    clause: "clause 2c16",
    from: "    if (reason) pause(reason)\n  } else {",
    to: "    if (reason) pause(reason)\n    if (event === 'StopFailure') writeTurnEnd(sessionId, recordPath)\n  } else {" },
  { name: "a StopFailure overwrites the Stop facts with unknowns (E8-R31, DS7-N2)", file: "dctr-cycle.mjs",
    clause: "clause 1x",
    from: "    if (reason) pause(reason)\n  } else {",
    to: "    if (event === 'StopFailure') writeMarker(stopFactsFile(sessionId), {})\n    if (reason) pause(reason)\n  } else {" },
  { name: "a submitted prompt goes on to alert and publish like the other events (E8-R31)", file: "dctr-cycle.mjs",
    clause: "clause 2c15",
    from: "  if (event === 'UserPromptSubmit') { log('turn end recorded'); process.exit(0) }\n",
    to: "" },
  { name: "a submitted prompt overwrites the Stop facts with unknowns (E8-R31)", file: "dctr-cycle.mjs",
    clause: "clause 2c15",
    from: "  if (event === 'UserPromptSubmit') { log('turn end recorded'); process.exit(0) }\n",
    to: "  if (event === 'UserPromptSubmit') { writeMarker(stopFactsFile(sessionId), {}); log('turn end recorded'); process.exit(0) }\n" },
  { name: "hooks.json runs no hook on UserPromptSubmit (E8-R31)", file: "hooks.json",
    clause: "clause 2r",
    from: '"UserPromptSubmit": [',
    to: '"UserPromptSubmitX": [' },
  { name: "a missing turn end frees every event pause (E8-R29)", file: "dctr-state.mjs",
    clause: "clause 2c14",
    from: "    return f.record === path.resolve(recordPath) && Number.isInteger(f.line) ? f.line : 0\n  } catch { return 0 }",
    to: "    return f.record === path.resolve(recordPath) && Number.isInteger(f.line) ? f.line : 0\n  } catch { return Infinity }" },
  { name: "a turn end written for another record frees lines in this one (RB6-2)", file: "dctr-state.mjs",
    clause: "clause 1w",
    from: "return f.record === path.resolve(recordPath) && Number.isInteger(f.line) ? f.line : 0",
    to: "return Number.isInteger(f.line) ? f.line : 0" },
  { name: "a damaged transcript line before the last is skipped as absence (RB6-1)", file: "dctr-lib.mjs",
    clause: "clause 2v",
    from: "    try { entries.push(JSON.parse(l)) } catch { return null }",
    to: "    try { entries.push(JSON.parse(l)) } catch { continue }" },
  { name: "a last transcript line still being written reads as nothing (RB6-1)", file: "dctr-lib.mjs",
    clause: "clause 2v",
    from: "catch { return { entries, partial: true } }",
    to: "catch { return { entries, partial: false } }" },
  { name: "the typer's step ignores a transcript mid-write (RB6-1)", file: "dctr-lib.mjs",
    clause: "clause 1j4",
    from: "  if (o.midWrite !== null && o.midWrite !== undefined) {",
    to: "  if (false) {" },
  { name: "the typer never tells its step a transcript is mid-write (RB6-1)", file: "dctr-typer.mjs",
    clause: "clause 2v",
    from: "      midWrite: partialSince === null ? null : now - partialSince,",
    to: "      midWrite: null," },
  { name: "a line of two spans unwraps as one, so prose quoting a form parses as it (DSP7-B1)", file: "dctr-record.mjs",
    clause: "clause 1o2",
    from: ".exec(t)) && !m[2].includes(m[1]); )",
    to: ".exec(t)); )" },
  { name: "the cycle hook's entry guard compares unresolved paths, so a symlinked plugin dir drops every event (ORC7-1)", file: "dctr-cycle.mjs",
    clause: "clause 2s",
    from: "const isMain = (() => { try { return fs.realpathSync(process.argv[1]) === fs.realpathSync(fileURLToPath(import.meta.url)) } catch { return false } })()",
    to: "const isMain = process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)" },
  { name: "the pane launcher's entry guard compares unresolved paths (ORC7-1)", file: "dctr-pane.mjs",
    clause: "no arguments through the link print the usage and exit 1",
    from: "const isMain = (() => { try { return fs.realpathSync(process.argv[1]) === fs.realpathSync(fileURLToPath(import.meta.url)) } catch { return false } })()",
    to: "const isMain = process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)" },
  { name: "the project CLI's entry guard compares unresolved paths (ORC7-1)", file: "dctr-project.mjs",
    clause: "CLI via a symlink",
    from: "const isMain = (() => { try { return fs.realpathSync(process.argv[1]) === fs.realpathSync(import.meta.filename) } catch { return false } })()",
    to: "const isMain = process.argv[1] && path.resolve(process.argv[1]) === import.meta.filename" },
  { name: "the typer types the bare resume line, not the doctrine-resume command (E8-R32, K1-D3b)", file: "dctr-lib.mjs",
    clause: "clause 1i",
    from: "export const RESUME_LINE = `${RESUME_COMMAND} ${RESUME_ARGS}`",
    to: "export const RESUME_LINE = `resume ${RESUME_ARGS}`" },
  { name: "the typer's own resume command entry counts as the user's typing, so every cycle pauses R16 (E8-R32)", file: "dctr-lib.mjs",
    clause: "clause 2a",
    from: "return !text.startsWith('<command-name>/clear</command-name>') && !ownResume(text)",
    to: "return !text.startsWith('<command-name>/clear</command-name>')" },
  { name: "any doctrine-resume command entry, whatever its arguments, reads as the typer's own (E8-R32)", file: "dctr-lib.mjs",
    clause: "clause 1j6",
    from: "tag('command-args')?.trim() === RESUME_ARGS && ",
    to: "" },
  { name: "the typer's command entry is recognised with other text beside its tags (E8-R32)", file: "dctr-lib.mjs",
    clause: "clause 1j6",
    from: " && !text.replace(/<(command-message|command-name|command-args)>[^<]*<\\/\\1>/g, '').trim()",
    to: "" },
  { name: "background tasks count as live by their number, not their status (E8-R33, K2-BG)", file: "dctr-cycle.mjs",
    clause: "clause 2c17",
    from: "      backgroundTasks: backgroundLive(payload.background_tasks),",
    to: "      backgroundTasks: nonEmpty(payload.background_tasks)," },
  { name: "the Stop facts count background tasks by their number, so an idle stop after ended tasks never pauses (E8-R33)", file: "dctr-cycle.mjs",
    clause: "clause 2c17",
    from: "backgroundEmpty: !backgroundLive(payload.background_tasks), cronsEmpty",
    to: "backgroundEmpty: !nonEmpty(payload.background_tasks), cronsEmpty" },
  { name: "a background task with a missing or unknown status reads as ended (E8-R33)", file: "dctr-lib.mjs",
    clause: "clause 1h5",
    from: "tasks.some((t) => !(t && typeof t === 'object' && BACKGROUND_ENDED.includes(t.status)))",
    to: "tasks.some((t) => t && typeof t === 'object' && t.status === 'running')" },
  { name: "a contained typer reserves its claim before it exits (RB9-2)", file: "dctr-typer.mjs",
    clause: "clause 2l",
    from: "if (process.env.DCTR_VIEW_REQUEST_DIR) process.exit(0)\n",
    to: "if (process.env.DCTR_VIEW_REQUEST_DIR) { try { const c = JSON.parse(process.argv[2]); reserveMarker(claimFile(c.session, c.keyLine)) } catch { /* none */ } process.exit(0) }\n" },
  { name: "endsReady reads the ready line only bare (K4-RL)", file: "dctr-lib.mjs",
    clause: "clause 2a3",
    from: "export const endsReady = (message) => unwrapLine(String(message ?? '').trimEnd().split('\\n').at(-1)) === READY_LINE",
    to: "export const endsReady = (message) => String(message ?? '').trimEnd().split('\\n').at(-1).trim() === READY_LINE" },
  { name: "the agent-written record forms are read only as written (K4-RL)", file: "dctr-record.mjs",
    clause: "clause 1o",
    from: "const m = re.exec(l) || (agent ? re.exec(unwrapLine(l)) : null)",
    to: "const m = re.exec(l)" },
  { name: "every record form is read through unwrapLine, the hook-written ones too (K4-RL)", file: "dctr-record.mjs",
    clause: "clause 1a",
    from: "const m = re.exec(l) || (agent ? re.exec(unwrapLine(l)) : null)",
    to: "const m = re.exec(l) || re.exec(unwrapLine(l))" },
  { name: "unwrapLine anchors the wrapper at the line's start only, so a span followed by other words reads as the form (K4-RL)", file: "dctr-record.mjs",
    clause: "clause 1h4",
    from: "(m = /^(`+|\\*\\*|\\*|__|_)(\\S(?:.*\\S)?)\\1$/.exec(t))",
    to: "(m = /^(`+|\\*\\*|\\*|__|_)(\\S(?:.*\\S)?)\\1/.exec(t))" },
  { name: "an idle prompt ignores the last Stop's session crons (RB6-3)", file: "dctr-lib.mjs",
    clause: "clause 1j5",
    from: " || f.lastStop?.cronsEmpty !== true",
    to: "" },
  { name: "the Stop facts carry no session crons (RB6-3)", file: "dctr-cycle.mjs",
    clause: "clause 2p",
    from: "cronsEmpty: !nonEmpty(payload.session_crons), ",
    to: "" },
  ...
  { name: "the typer never reads the new transcript for typing (DP-1)", file: "dctr-typer.mjs",
    clause: "clause 2u",
    from: "typedNew: fresh === null ? null : fresh.entries.some(userTyped),",
    to: "typedNew: false," },
  ...
  { name: "the cycle line is written on the /clear send, so a /clear that did not take still counts a cycle (E8-D28)", file: "dctr-typer.mjs",
    clause: "clause 2m2",
    from: "      send('/clear')\n      log(`sent /clear, cycle ${a.n}`)",
    to: "      send('/clear')\n      appendRecordLine(a.record, `- auto-cycle: cycle ${a.n} tree ${a.hash}`)\n      log(`sent /clear, cycle ${a.n}`)" },
  { name: "herdr still reporting the old session after /clear reads as R14, not a /clear that did not take (E8-D28)", file: "dctr-lib.mjs",
    clause: "clause 1f2",
    from: "    return s === o.oldSession ? pause('R18', o.oldSession) : pause('R14')",
    to: "    return pause('R14')" },
  ...
  { name: "only the restore file proves a /clear took, so herdr reporting the new session counts no cycle (E8-D17, E8-D28)", file: "dctr-lib.mjs",
    clause: "clause 1f3",
    from: "  const took = Boolean(o.restore) || (s !== null && s !== o.oldSession)",
    to: "  const took = Boolean(o.restore)" },
  { name: "a failed lookup past the restore wait reads as R14, a clear nobody saw (E8-D28)", file: "dctr-lib.mjs",
    clause: "clause 1f5",
    from: "    if (s === null) return pause('R17', R17_WHY.lookup)\n",
    to: "" },
  { name: "the typer ignores the cycle flag and appends the cycle line on the restore file only (E8-D17, E8-D28)", file: "dctr-typer.mjs",
    clause: "clause 2m",
    from: "    if (step.cycle) {",
    to: "    if (restore && !cycled) {" },
  { name: "typerStep's session reading accepts an empty string as a session (RT2-B1)", file: "dctr-lib.mjs",
    clause: "clause 1f3",
    from: " && pane.session !== '' ? pane.session : null)",
    to: " ? pane.session : null)" },
  { name: "typerStep's lookup guard accepts an empty session string, so it reads as a changed session (RT2-B1)", file: "dctr-lib.mjs",
    clause: "clause 1e7",
    from: "  if (paneSession(o.pane) === null) return pause('R17', R17_WHY.lookup)",
    to: "  if (!o.pane || o.pane.error || typeof o.pane.session !== 'string') return pause('R17', R17_WHY.lookup)" },
  { name: "the Stop hook takes an empty session string as a session, so it pauses R12 (RT2-B1)", file: "dctr-cycle.mjs",
    clause: "clause 2c18",
    from: "          if (typeof value === 'string' && value !== '') return value\n",
