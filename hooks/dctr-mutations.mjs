...
const MUTATIONS = [
  ...
  { name: 'the watcher relabels with the status', file: 'dctr-seat.mjs', clause: 'relabels the pane with the status',
    ...
    to: '' },
  { name: 'the watcher waits while the job runs', file: 'dctr-seat.mjs', clause: 'the watcher exits once the job is no longer running',
    from: "    if (!cur || cur.status === 'running') return",
    to: '    return' },
]
