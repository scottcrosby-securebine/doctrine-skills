...
const MUTATIONS = [
  ...
  { name: "a StopFailure records its turn end after its own pause, so a repeat error is refused (E8-R31, DP6-B1)", file: "dctr-cycle.mjs",
    clause: "clause 2c15",
    from: "  if (event !== 'Notification') writeTurnEnd(sessionId, recordPath)\n  if (event === 'UserPromptSubmit')",
    to: "  if (event === 'UserPromptSubmit') writeTurnEnd(sessionId, recordPath)\n  if (event === 'UserPromptSubmit')" },
  { name: "a submitted prompt goes on to alert and publish like the other events (E8-R31)", file: "dctr-cycle.mjs",
    ...
    to: "" },
  { name: "a StopFailure or submitted prompt overwrites the Stop facts with unknowns (E8-R31)", file: "dctr-cycle.mjs",
    clause: "clause 2c15",
    from: "  if (event === 'UserPromptSubmit') { log('turn end recorded'); process.exit(0) }\n",
  ...
  { name: "Stop facts written for another record free lines in this one (RB6-2)", file: "dctr-state.mjs",
    clause: "clause 1w",
    from: "return f.record === path.resolve(recordPath) && Number.isInteger(f.line) ? f.line : 0",
