...
const MUTATIONS = [
  ...
  { name: "the Stop decision drops its guard: an ordinary turn with no warned line (E8-D7)", file: "dctr-lib.mjs",
    ...
    to: "" },
  { name: "the Stop decision drops its guard: a paused line after this session's warned line (E8-D7)", file: "dctr-lib.mjs",
    clause: "clause 1e",
    from: "  if (f.pausedAfterWarned) return { act: 'none', reason: \"a paused line follows this session's warned line\" }\n",
    to: "" },
  ...
  { name: "the Stop decision drops its guard: R17 a failed pane lookup (E8-D7)", file: "dctr-lib.mjs",
    clause: "clause 1e",
    from: "  if (session?.error) return pause('R17', session.error)\n",
    to: "" },
  ...
  { name: "the typer sends to a pane that is not idle (E8-D15)", file: "dctr-lib.mjs",
    clause: "clause 1e",
    from: "  if (o.pane.status !== 'idle') {",
    to: "  if (false) {" },
  ...
  { name: "a paused line is written over one already standing (E8-D18)", file: "dctr-state.mjs",
    clause: "clause 1k",
    from: "  if (latestAutoCycle(parseRecord(fs.readFileSync(recordPath, 'utf8')).entries)?.sub === 'paused') return { written: false }\n",
    to: "" },
  ...
  { name: "a paused line is alerted again at every event (F2)", file: "dctr-state.mjs",
    clause: "clause 2i",
    from: "  try { reserveMarker(path.join(autoCycleDir(), `alerted-${hash}-${latest.line}`)) } catch { return null }\n",
    to: "" },
  ...
  { name: "the on token is published over a standing pause (LB2)", file: "dctr-cycle.mjs",
    clause: "clause 2k",
    from: "    if (latestAutoCycle(entries)?.sub !== 'paused') {",
    to: "    if (true) {" },
