...
const MUTATIONS = [
  ...
  { name: "the standing pauses are read from the latest line only, so the ready line hides a pause (RB2-3)", file: "dctr-lib.mjs",
    clause: "clause 1q",
    from: "  if (anchor) return unresolvedPausesAfter(es, anchor.line)\n",
    to: "" },
  { name: "the standing pauses ignore the cycle line as an anchor", file: "dctr-lib.mjs",
    clause: "clause 1q",
    from: "(e.sub === 'warned' || e.sub === 'cycle')",
    to: "e.sub === 'warned'" },
  { name: "the dedup holds on any latest paused line, answered or not (E8-R25)", file: "dctr-lib.mjs",
    clause: "clause 2c5",
    from: "  return latest?.sub === 'paused' && standingPauses(entries).includes(latest)",
    to: "  return latest?.sub === 'paused'" },
  { name: "a paused line is written over one already standing (E8-D18)", file: "dctr-state.mjs",
    clause: "clause 1k",
    from: "  if (pauseStands(parseRecord(fs.readFileSync(recordPath, 'utf8')).entries)) return { written: false }\n",
    to: "" },
  ...
  { name: "the pane message is printed at every event (E8-R23)", file: "dctr-state.mjs",
    ...
    to: "return true" },
  { name: "the claim is keyed by session alone, so a new ready line after a resolved pause never launches (RB2-2)", file: "dctr-state.mjs",
    clause: "clause 2c9",
    from: "path.join(autoCycleDir(), `${sessionId}.${readyLine}.claim`)",
  ...
  { name: "the Stop hook tests the claim for the session's first ready line, not the latest (RB2-2)", file: "dctr-cycle.mjs",
    clause: "clause 2c9",
    from: ".filter((e) => e.kind === 'auto-cycle' && e.sub === 'ready').at(-1)?.line ?? 0",
    to: ".filter((e) => e.kind === 'auto-cycle' && e.sub === 'ready').at(0)?.line ?? 0" },
  { name: "the typer runs without its claim (E8-D15)", file: "dctr-typer.mjs",
    clause: "clause 2b",
    from: "  try { reserveMarker(claimFile(a.session, a.readyLine)) } catch",
    to: "  try { } catch" },
  ...
  { name: "the Stop hook hands the typer no Stop time (K2-R16)", file: "dctr-cycle.mjs",
    clause: "clause 2q",
    from: "phase: header.phase, stopAt, readyLine }",
    to: "phase: header.phase, readyLine }" },
