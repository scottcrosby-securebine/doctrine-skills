...
const MUTATIONS = [
  ...
  { name: "the Stop decision drops its guard: R5 the cycle cap (E8-D7)", file: "dctr-lib.mjs",
    ...
    to: "" },
  { name: "the Stop decision drops its guard: this session's claim already taken (E8-D7)", file: "dctr-lib.mjs",
    clause: "clause 1e",
    from: "  if (f.claimTaken) return { act: 'none', reason: \"this session's claim is already taken\" }\n",
    to: "" },
  ...
  { name: "a reason with no argument is recognised by nothing, so its action is lost (F9)", file: "dctr-lib.mjs",
    ...
    to: "false" },
  { name: "a Blocked pause never resolves (E8-R25)", file: "dctr-lib.mjs",
    clause: "clause 1p",
    from: "  if (PAUSES.R2.match.test(reason)) return !/^Blocked",
    to: "  if (PAUSES.R2.match.test(reason)) return false && !/^Blocked" },
  { name: "an alarm pause never resolves (E8-R25)", file: "dctr-lib.mjs",
    clause: "clause 1p",
    from: "  if (alarm) return !es.some(",
    to: "  if (alarm) return false && !es.some(" },
  { name: "a question pause never resolves (E8-R25)", file: "dctr-lib.mjs",
    clause: "clause 1p",
    from: "    return es.some((e) => e.kind === 'question-answered' && e.id === q[1] && e.line > opened)",
    to: "    return false" },
  { name: "a question pause resolves on another question's answer (E8-R25)", file: "dctr-lib.mjs",
    clause: "clause 1p",
    from: "e.kind === 'question-answered' && e.id === q[1] && e.line > opened",
    to: "e.kind === 'question-answered' && e.line > opened" },
  { name: "every other pause resolves (E8-R25)", file: "dctr-lib.mjs",
    clause: "clause 1p",
    from: "  return false\n}\n\n/** The record's standing pause",
    to: "  return true\n}\n\n/** The record's standing pause" },
  { name: "a resolved pause still stands, so no fresh pause is written and the token stays paused (E8-R25)", file: "dctr-lib.mjs",
    clause: "clause 2c5",
    from: "latest?.sub === 'paused' && !pauseResolved(latest, entries) ? latest : null",
    to: "latest?.sub === 'paused' ? latest : null" },
  { name: "a resolved pause still holds step 3 (E8-R25)", file: "dctr-lib.mjs",
    clause: "clause 2c4",
  ...
  { name: "a paused line is written over one already standing (E8-D18)", file: "dctr-state.mjs",
    clause: "clause 1k",
    from: "  if (standingPause(parseRecord(fs.readFileSync(recordPath, 'utf8')).entries)) return { written: false }\n",
    to: "" },
  ...
  { name: "a paused line is alerted again at every event (F2)", file: "dctr-state.mjs",
    clause: "clause 2i",
    from: "  try { reserveMarker(pauseMarker('alerted', recordPath, standing.line)) } catch { return false }\n",
    to: "" },
  { name: "the pane message is printed at every event (E8-R23)", file: "dctr-state.mjs",
    clause: "clause 2i",
    from: "  try { reserveMarker(pauseMarker('shown', recordPath, standing.line)) } catch { return null }\n",
    to: "" },
  ...
  { name: "a paused line anywhere counts as following the warned line", file: "dctr-cycle.mjs",
    clause: "clause 2c2",
    from: "unresolvedPauseAfter(record.entries, mine.line)",
    to: "unresolvedPauseAfter(record.entries, 0)" },
  ...
  { name: "a Notification acts while the stop file stands (B6)", file: "dctr-cycle.mjs",
    ...
    from: "  } else if (active) {",
    to: "  } else if (true) {" },
  { name: "the on token is published over a standing pause (LB2)", file: "dctr-cycle.mjs",
    clause: "clause 2k",
    from: "  if (active && paneId && !standingPause(entries)) {",
    to: "  if (active && paneId) {" },
  ...
  { name: "the typer runs without its claim (E8-D15)", file: "dctr-typer.mjs",
    clause: "clause 2b",
    from: "  try { reserveMarker(claimFile(a.session)) } catch",
    to: "  try { } catch" },
  ...
  { name: "the typer runs with no Stop time (K2-R16)", file: "dctr-typer.mjs",
    clause: "clause 2f3",
    from: " || !Number.isFinite(a.stopAt)) process.exit(0)",
    to: ") process.exit(0)" },
  { name: "the Stop hook hands the typer no Stop time (K2-R16)", file: "dctr-cycle.mjs",
    clause: "clause 2q",
    from: "phase: header.phase, stopAt }",
    to: "phase: header.phase }" },
]
