...
**Hub and spokes.** `skills/doctrine/SKILL.md` is the hub: the seven-step posture (ask questions first, phases and waves of parallel agents, native checks, red team, loop to one clean pass, simplify, deliver) and the fallback table for missing prerequisites. Nine of the ten skills are `doctrine-*`; the eight **wrappers** each begin with "REQUIRED BACKGROUND: read the `doctrine` skill first" and supply only the task shape: which external skill is the core discipline, how phases map onto it, and which review skill the phase gate uses. Don't restate posture steps inside a wrapper — reference them ("doctrine step 4").
