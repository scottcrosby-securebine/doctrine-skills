...
**Gate semantics are load-bearing.** The two-clean-pass exit gate, the blocking/non-blocking split that defines a "finding" (blocking = the deliverable is wrong or unfinished without a change to it; non-blocking improvements, style nitpicks declined with a reason, findings the user rules closed, and red-team claims verified false from source don't reset the counter), the four-loop escalation valve, the four terminal states a phase can end in (only one of them clean), the fresh/fallout label on blocking findings with its fix-the-class rule, the build-the-check-first rule (which the hub applies to **any** mechanically catchable class, not only to fallout), and the diagnostic form of the valve's escalation are deliberate design (see README's posture step 5 and Design notes) — don't loosen them casually when editing skills. The terminal states, the fallout label and the diagnostic escalation were added after a six-round run in this repo whose rounds 5 and 6 found no code defect and kept finding prose the earlier rounds' own fixes had made stale: the gate was right to count each one, and wrong only in that nothing cheaper than a review round could see them.
